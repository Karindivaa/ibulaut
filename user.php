...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (isset($_POST['beli_sekarang'])) {
    $id = $_POST['produk_id'];
    $_SESSION['checkout'] = [$id => 1]; // hanya produk itu yang dibeli
...
<header class="navbar">
    <div class="container">
      ...
      <div class="nav-actions">
        <a id="toggleCart"class="btn-primary"> 🛒 Keranjang</a>
  ...
  <?php 
    ...
    <a href="checkout.php" style="display: block; text-align: center; background: #28a745; color: white; padding: 10px; border-radius: 6px; text-decoration: none;">Beli Sekarang</a>
  <?php else: ?>
...
<section class="produk-unggulan">
  ...
  <form method="POST">
    <input type="hidden" name="produk_id" value="<?= $index ?>">
    <button class="btn-beli" name="add_to_cart">🛒 Tambahkan Ke Keranjang</button>
  </form>
</div>
...
<form method="POST">
  <input type="hidden" name="produk_id" value="<?= $index ?>">
  <button type="submit" class="btn-beli-outline" name="beli_sekarang">Beli Sekarang</button>
</form>
